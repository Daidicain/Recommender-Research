...
@njit(parallel=False)
def get_subsets( items: np.ascontiguousarray, timestamps:np.ascontiguousarray, rating:np.ascontiguousarray, t_window: int, size: int) -> np.array:
    '''
...
def initialize_structures(train: np.array, unique_users: np.array, t_window: int, **kwargs) -> pd.DataFrame:
    ...
    for user in unique_users:
        ...
        for subset, subset_rating in get_subsets(items, timestamps, ratings, t_window, len(user_data)):
...
def recommender_algorithm(context_df: pd.DataFrame, train: pd.DataFrame, user: str, k: int, **kwargs) -> dict:
    ...
    user_items = context_df[ context_df['user_id'] == user ]
    user_context_set = set(user_items['context'])
    ...
    for user_context in user_context_set:
        for context in related_context:
            
            user_items_set = set(user_items[ user_items['context'] == user_context ]['item_id'].unique())
            ...
            context_items = context_items_table['item_id'].unique()
            # print('table', time.time() - t)
